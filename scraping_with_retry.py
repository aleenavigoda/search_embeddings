...
def scrape_content(url, retries=3, backoff_factor=2):
...
# Function to fetch entries with error codes from Supabase with pagination
def get_entries_with_errors_paginated(supabase, limit=50, page=0):
    try:
        # Paginate through the entries by applying an offset
        data = supabase.table('urls_table') \
            .select('id', 'url', 'full_text') \
            .filter('full_text', 'ilike', 'Error Code:%') \
            .range(page * limit, (page + 1) * limit - 1) \
            .execute()

        return data.data  # Return the list of records with errors
    except Exception as e:
        print(f"Failed to fetch entries with errors: {e}")
        return []


# Function to retry failed entries with pagination
def retry_failed_entries_paginated(supabase, limit=50):
    page = 0
    while True:
        error_records = get_entries_with_errors_paginated(supabase, limit, page)
        
        # If no more records, break the loop
        if not error_records:
            break

        print(f"Processing page {page + 1} with {len(error_records)} entries with errors.")
        
        # Use ThreadPoolExecutor to parallelize the retry process
        with ThreadPoolExecutor(max_workers=50) as executor:  # Adjust max_workers based on your need
            futures = []
            
            # Submit tasks to the executor for retrying
            for index, record in enumerate(error_records):
                futures.append(executor.submit(retry_and_update, record, supabase, index))
...
# Function to retry scraping for a single record
def retry_and_update(record, supabase, index):
    if isinstance(record, dict) and 'id' in record and 'url' in record:
        record_id = record['id']
        url = record['url']
        print(f"Retrying URL {index + 1}: {url}")  # Print the index and the URL
        
        # Scrape content
        full_text = scrape_content(url)

        if full_text:
            # Update the full_text column in Supabase if scraping succeeds
            update_full_text_in_supabase(supabase, record_id, full_text)
            log_success_to_csv(url, record_id)  # Log success
            print(f"Successfully updated full_text for record ID {record_id} after retry")
        else:
            print(f"Skipping update for record ID {record_id} (Error Code persists)")
    else:
        print(f"Skipping invalid record: {record}")
...
def main():
    ...
    total_count, urls_records = retry_failed_entries_paginated(supabase, 1000)
...
# Main function to call retry_failed_entries
def main_retry():
    initialize_logs()  # Initialize the error and success log files
    supabase = connect_to_supabase()  # Initialize Supabase with provided URL and API key
    
    # Retry failed entries
    retry_failed_entries(supabase, limit=100)

if __name__ == "__main__":
    main()
    # Uncomment the next line if you want to retry failed entries
    main_retry()
